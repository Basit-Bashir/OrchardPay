...
export async function requestOtp(mobile: string): Promise<OtpRequestResult> {
  ...
  // In console/dev mode or when the API key is missing, return the OTP so the UI can show it for easy testing.
  const devOtp = (process.env.SMS_PROVIDER ?? "console") === "console" || !process.env.FAST2SMS_API_KEY ? otp : undefined;
